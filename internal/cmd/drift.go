...
// generationDependencies lists files that templates reference during generation.
// These must be copied to the in-memory filesystem during drift detection to
// ensure consistent hash comparison between actual and expected generated files.
//
// Includes:
// - User-customizable files that templates read (e.g., docs/README.md)
// - External tool outputs that templates consume (e.g., .webkit/outputs.json from Terraform)
//
// Without copying these files, generated output differs between actual filesystem
// and in-memory generation, causing false positive drift detection.
var generationDependencies = []string{
	"docs/AGENTS.md",
	"docs/AGENTS.md.tmpl",
	"docs/README.md",
	".webkit/outputs.json",
...
func drift(ctx context.Context, input cmdtools.CommandInput) error {
	...
	if err := copyGenerationDependencies(input.FS, memFS); err != nil {
		return errors.Wrap(err, "copying generation dependencies to memory")
	}
...
// copyGenerationDependencies copies files that templates reference during generation
// to the in-memory filesystem. Without these files, generated output differs from
// actual files, causing false drift warnings.
//
// This includes both user-customizable files and external tool outputs that templates
// consume during generation (e.g., Terraform outputs, custom documentation).
func copyGenerationDependencies(srcFS, dstFS afero.Fs) error {
	for _, path := range generationDependencies {
