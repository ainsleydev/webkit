...
func TestGenerate(t *testing.T) {
	...
	t.Run("With custom content from docs/AGENTS.md", func(t *testing.T) {
		...
		err := afero.WriteFile(fs, customContentPath+"/AGENTS.md", []byte(customContent), 0o644)
		require.NoError(t, err)
	...
	t.Run("With custom template from docs/AGENTS.md.tmpl", func(t *testing.T) {
		...
		err := afero.WriteFile(fs, customContentPath+"/AGENTS.md.tmpl", []byte(customTemplate), 0o644)
		require.NoError(t, err)
