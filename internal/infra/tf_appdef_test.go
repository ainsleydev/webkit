...
func TestTerraform_TerraformManaged(t *testing.T) {
	t.Run("Resource not managed", func(t *testing.T) {
		notManaged := false
		appDef := &appdef.Definition{
			Project: appdef.Project{
				Name: "project",
		...
		t.Log("Skipped Items")
		{
			assert.Len(t, got.Skipped.Resources, 1, "Should skip 1 resource")
			assert.Contains(t, got.Skipped.Resources, "db")
			assert.Len(t, got.Skipped.Apps, 0, "Should skip 0 apps")
		}

		t.Log("Resource Names Output")
		{
			resourceNames := got.Plan.PlannedValues.Outputs["resource_names"]
			require.NotNil(t, resourceNames)
			names := resourceNames.Value.([]any)
			assert.Len(t, names, 0, "Should have no managed resources in output")
		}

		t.Log("No Database Resources")
		{
			for _, rc := range got.Plan.ResourceChanges {
				assert.NotEqual(t, "digitalocean_database_cluster", rc.Type, "Should not plan database cluster")
				assert.NotEqual(t, "digitalocean_database_db", rc.Type, "Should not plan database")
				assert.NotEqual(t, "digitalocean_database_user", rc.Type, "Should not plan database user")
			}
		}
	})

	t.Run("App not managed", func(t *testing.T) {
		t.Skip("Apps test not ready")
		...
		t.Log("Skipped Items")
		{
			assert.Len(t, got.Skipped.Apps, 1, "Should skip 1 app")
			assert.Contains(t, got.Skipped.Apps, "web")
			assert.Len(t, got.Skipped.Resources, 0, "Should skip 0 resources")
		}

		t.Log("No App Resources")
		{
			for _, rc := range got.Plan.ResourceChanges {
				assert.NotEqual(t, "digitalocean_app", rc.Type, "Should not plan DigitalOcean app")
			}
		}
	})
	...
	t.Run("Mixed managed and unmanaged", func(t *testing.T) {
		...
		{
			assert.Len(t, got.Skipped.Resources, 1, "Should skip 1 resource")
			assert.Contains(t, got.Skipped.Resources, "legacy-db")
			assert.NotContains(t, got.Skipped.Resources, "db")
			assert.Len(t, got.Skipped.Apps, 0, "Should skip 0 apps")
		}
		...
		t.Log("Resource Names Output")
		{
			...
			require.NotNil(t, resourceNames)
			names := resourceNames.Value.([]any)
			assert.Len(t, names, 1, "Should have 1 managed resource in output")
			assert.Contains(t, names, "db")
			assert.NotContains(t, names, "legacy-db")
		}

		t.Log("Only Managed Database Resources")
		{
			var dbClusterCount int
			for _, rc := range got.Plan.ResourceChanges {
				if rc.Type == "digitalocean_database_cluster" {
					dbClusterCount++
					dbCluster := rc.Change.After.(map[string]any)
					assert.Equal(t, "project-db", dbCluster["name"], "Only managed db should be planned")
				}
			}
			assert.Equal(t, 1, dbClusterCount, "Should plan exactly 1 database cluster")
		}
	})

	t.Run("Default behaviour nil equals managed", func(t *testing.T) {
		appDef := &appdef.Definition{
			Project: appdef.Project{
				Name: "project",
				Repo: appdef.GitHubRepo{
		...
		t.Log("Database Resources Planned")
		{
			var dbCluster map[string]any
			for _, rc := range got.Plan.ResourceChanges {
				if rc.Type == "digitalocean_database_cluster" && rc.Name == "this" {
			...
			require.NotNil(t, dbCluster, "Database cluster should be planned when terraformManaged is nil")
			assert.Equal(t, "project-db", dbCluster["name"])
		}
	})

	t.Run("Explicitly true equals managed", func(t *testing.T) {
		managed := true
		appDef := &appdef.Definition{
			Project: appdef.Project{
				Name: "project",
		...
		t.Log("Database Resources Planned")
		{
			...
			require.NotNil(t, dbCluster, "Database cluster should be planned when terraformManaged is true")
			assert.Equal(t, "project-db", dbCluster["name"])
		}
	})
}
