...
{{- if and .Outputs .Outputs.Monitoring.StatusPageURL }}
[![Status]({{ .Outputs.Monitoring.PeekapingEndpoint }}/api/v1/badge/{{ index (index .Outputs.Monitoring.HTTPMonitors (keys .Outputs.Monitoring.HTTPMonitors | first)) "id" }}/status)]({{ .Outputs.Monitoring.StatusPageURL }})
{{- end }}
