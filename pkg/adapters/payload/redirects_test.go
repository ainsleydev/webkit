...
import (
	...
	"github.com/ainsleydev/webkit/pkg/apis/payloadcms/payloadfakes"
	...
	tt := map[string]struct {
		...
		"Invalid number defaults to 301": {
			mock: func(_ *payloadfakes.MockCollectionService, store cache.Store) {
				err := store.Set(context.Background(), redirectCacheKey, []Redirect{
		...
		"Redirects 301 from Cache": {
			mock: func(_ *payloadfakes.MockCollectionService, store cache.Store) {
				err := store.Set(context.Background(), redirectCacheKey, redirects, cache.Options{})
				require.NoError(t, err)
			},
			wantStatus: http.StatusMovedPermanently,
			wantURL:    "/new",
